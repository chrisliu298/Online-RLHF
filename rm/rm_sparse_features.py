...
class Qwen2ForSequenceClassificationWithSparseFeatures(Qwen2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = Qwen2Model(config)
        self.sparse_encoder = nn.Linear(config.hidden_size, config.sparse_hidden_size)
        self.score = nn.Linear(config.K, self.num_labels, bias=False)
    ...
    def forward(
        ...
        pooled_logits = self.score(top_k_features)
